...
export default function LessonDetail() {
  ...
  if (!lesson)
    return <div className="p-8 text-center text-ios-gray4">Dars topilmadi</div>;
  ...
  return (
    <div className="p-5 lg:p-8 max-w-[1100px]">
      <Link
        ...
        className="inline-flex items-center gap-1.5 text-sm font-semibold text-ios-label2 bg-ios-gray1 rounded-full px-4 py-2 mb-5 hover:bg-ios-gray2 transition-colors"
      >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_268px] gap-5">
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl p-6 mb-4">
            <span className="inline-block bg-blue-50 text-ios-blue text-xs font-bold px-3 py-1 rounded-full mb-3">
              {lesson.courseId.charAt(0).toUpperCase() +
                lesson.courseId.slice(1)}{" "}
              · Dars #{lesson.num}
            </span>
            <h1 className="text-2xl font-extrabold tracking-tight text-ios-label mb-2">
              {lesson.title}
            </h1>
            <p className="text-sm text-ios-gray4">
              React nima, nima uchun kerak va qanday o'rnatiladi — barchasi shu
              darsda.
            </p>
            <div className="flex flex-wrap gap-2 mt-4">
              ...
              <span className="bg-ios-gray1 text-ios-gray4 text-xs rounded-full px-3 py-1.5">
                📝 {quiz.length} savol
              </span>
              <span className="bg-ios-gray1 text-ios-gray4 text-xs rounded-full px-3 py-1.5">
                🛠 3 mashq
              ...
              <span className="bg-ios-gray1 text-ios-gray4 text-xs rounded-full px-3 py-1.5">
                🔤 {lesson.vocabulary?.length ?? 0} so'z
              </span>
              {completed && (
                <span className="bg-green-50 text-ios-green text-xs rounded-full px-3 py-1.5 font-semibold">
                  ✓ Tugallandi
                </span>
              )}
            </div>
              ...
              <button
                ...
                className={`shrink-0 px-4 py-2 rounded-full text-[13px] font-semibold transition-all ${
                  activeTab === id
                    ...
                    : "bg-white text-ios-gray4 hover:bg-ios-gray2"
                }`}
              >
                {label}
              </button>
            ...
            <div className="space-y-4">
              ...
              {lesson.strategies?.length > 0 && (
                <div className="bg-white rounded-2xl p-5">
                  <p className="text-[11px] font-bold uppercase tracking-wider text-ios-gray4 mb-3">
                    Strategies & Ideas
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {lesson.strategies.map((s, i) => (
                      <span
                        key={s}
                        className={`px-4 py-1.5 rounded-full text-sm font-semibold ${chipColors[i % chipColors.length]}`}
                      >
                        {s}
                      </span>
                    ))}
              ...
              {lesson.content?.length > 0 && (
                <div className="bg-white rounded-2xl p-5">
                  <p className="text-[11px] font-bold uppercase tracking-wider text-ios-gray4 mb-4">
                    Things to cover
                  </p>
                  {lesson.content.map(({ heading, bullets }, i) => (
                    <div key={i} className={i > 0 ? "mt-5" : ""}>
                      <p className="text-[15px] font-bold text-ios-label mb-2.5">
                        {heading}
                      </p>
                      <ul className="space-y-2">
                        {bullets.map((b, j) => (
                          <li key={j} className="flex items-start gap-2.5">
                            ...
                            <span
                              className="text-[15px] text-ios-label2 leading-relaxed"
              ...
              <div className="flex gap-2.5 pt-1">
                <button className="bg-ios-gray1 text-ios-label rounded-full px-5 py-2.5 text-sm font-semibold hover:bg-ios-gray2 transition-colors">
                  ← Oldingi dars
                </button>
                <button
          ...
          {activeTab === "quiz" && (
            <QuizTab questions={quiz} lessonId={lessonId} />
          )}
        ...
        <div className="hidden xl:flex flex-col gap-4 sticky top-20 self-start">
          {/* Next lesson */}
          <div className="bg-white rounded-2xl p-5">
            <p className="text-[11px] font-bold uppercase tracking-wider text-ios-gray4 mb-2.5">
              Keyingi dars
            </p>
            <p className="text-sm font-bold text-ios-label mb-1">
              Components & Props
            </p>
            <p className="text-xs text-ios-gray4 mb-3">
              {lesson.courseId} · ~30 daqiqa
            </p>
            <button
              disabled
              className="w-full bg-ios-gray2 text-ios-gray4 rounded-full py-2 text-xs font-semibold opacity-70 cursor-not-allowed flex items-center justify-center gap-1.5"
            >
          ...
          {/* TOC */}
          <div className="bg-white rounded-2xl p-5">
            <p className="text-sm font-bold text-ios-label mb-3">
              Dars tarkibi
            </p>
            {TABS.map(({ id, label }) => (
              <button
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full shrink-0 ${activeTab === id ? "bg-ios-blue" : "bg-ios-gray3"}`}
                />
                {label}
              </button>
            ))}
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={label} className="flex justify-between text-sm">
                  ...
                  <span
                    className={
                      done ? "text-ios-green font-semibold" : "text-ios-gray4"
                    }
                  >
                    {done ? "✓" : "—"}
                  </span>
